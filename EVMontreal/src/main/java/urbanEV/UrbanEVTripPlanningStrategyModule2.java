...
public class UrbanEVTripPlanningStrategyModule2 implements PlanStrategyModule{
	...
	protected static final Logger log = Logger.getLogger(UrbanEVTripPlanningStrategyModule2.class);
	protected static List<PersonContainer2> personContainer2s = new ArrayList<>();
//	private boolean ifDumbCharing = true;
	private double dumbChargingStartTime = 1*3600;
	private double dumbChargingEndTime = 23*3600;
	...
	@Inject
	UrbanEVTripPlanningStrategyModule2(){
...
//			if(this.ifDumbCharing) {
				...
				for (PlanElement pel: new ArrayList <> (modifiablePlan.getPlanElements())) {
					if (pel instanceof Activity && ((Activity) pel).getStartTime().isDefined() && !pel.equals(modifiablePlan.getPlanElements().get(0)) && !pel.equals(modifiablePlan.getPlanElements().get(modifiablePlan.getPlanElements().size()-1)) && ((Activity) pel).getStartTime().seconds() >= this.dumbChargingStartTime && ((Activity) pel).getStartTime().seconds() <= this.dumbChargingEndTime && pseudoVehicle.getBattery().getSoc()/pseudoVehicle.getBattery().getCapacity() <.30) {
...
//				int ind = random.nextInt(pe.size());
				...
				planPluginTrip(modifiablePlan, routingMode, electricVehicleSpecification, pluginTripOrigin, (Activity) act, chargingLink, tripRouter);
				planPlugoutTrip(modifiablePlan, routingMode, electricVehicleSpecification, act, plugoutTripDestination, chargingLink, tripRouter, PlanRouter.calcEndOfActivity(act, modifiablePlan, config));
				cnt --;
...
private Map<Id<Person>, List<Plan>> pel() {
		// TODO Auto-generated method stub
		return null;
	}
