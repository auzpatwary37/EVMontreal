...
public class UrbanEVTripPlanningStrategyModule implements PlanStrategyModule{
	...
	public void handlePlan(Plan plan) {
		...
		for (Id<Vehicle> ev : evs) {
			...
			if(pluginBeforeStart && hasHomeCharger(plan.getPerson(), modifiablePlan, evCarLegs, pseudoVehicle) && modifiablePlan.getPlanElements().indexOf(evCarLegs.get(0))==1){ //TODO potentially check for activity duration and/or SoC
	...
	protected void replanPrecedentAndCurrentEVLegs(Plan modifiablePlan, ElectricVehicleSpecification electricVehicleSpecification, Leg leg) {
		...
		do {
			actWhileCharging = activityWhileChargingFinder.findActivityWhileChargingBeforeLeg(modifiablePlan, (Leg) modifiablePlan.getPlanElements().get(legIndexCounter));
			...
			selectedCharger = selectChargerNearToLink(modifiablePlan.getPerson().getId(),actWhileCharging.getLinkId(), electricVehicleSpecification, modeNetwork);
